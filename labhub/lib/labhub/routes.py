...
from labhub.lib.forms import LoginForm, AddMeasurementLog, RegistrationForm, UpdateAccountForm, AddSetup, AddSample, AddProject, Attribute, AddStructure, AddSession, FilterSession, AddOccasion, LimitOccs, AddLocationSample, FilterLogs, Dispersion, addRemarkToLog, DispersionWaveguide, LabSensorDay
from labhub.lib.pagination import Pagination
from labhub.lib.models import User, Log, LogImages, Setup, SetupImages, Sample, SampleImages, Project, Structure, StructureImages, Session, LogCooperators, SampleLocations, SetupFiles, LogRemark
...
def addSample():
    ...
    if form.validate_on_submit():
        ...
        for entry in form.attr.entries:
            attributes = attributes + entry.data['attrName'] + ',' + entry.data['attrValue'] + '\n'
        sample = Sample(name=form.name.data, desc=form.desc.data, attribute=attributes)
...
@login_required
def list_sample():
    sample = Sample.query.order_by(asc('name')).all()
...
def project(project_id):
    ...
    return render_template('project.html', title=project.name, project=project, sessions=sessions)
...
def _dispersionWaveguide():
    ...
    if int(request.values.get('extFieldUnits')) == 0:
        ...
        H0 = float(request.values.get('extField'))
    if int(request.values.get('exchangeUnits')) == 0:
        A = float(request.values.get('exchange'))*2/(Msat**2*mu0)
    ...
    print(A)
    # Damon-Eshbach - 00
    fDE =  slavinDispWaveguide(kxi=kxi, theta=theta, phi=np.pi/2, n=0, d=d, w0=w0, wM=wM, A=A, weff=weff, nT=nT, boundaryCond=boundaryCond)/np.pi/2
    # Backward volume - 00
    fBV =  slavinDispWaveguide(kxi=kxi, theta=theta, phi=0, n=0, d=d, w0=w0, wM=wM, A=A, weff=weff, nT=nT, boundaryCond=boundaryCond)/np.pi/2
    # Custom
    fc =  slavinDispWaveguide(kxi=kxi, theta=theta, phi=phi, n=n, d=d, w0=w0, wM=wM, A=A, weff=weff, nT=nT, boundaryCond=boundaryCond)/np.pi/2
